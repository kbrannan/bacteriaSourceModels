...
post.merge.pls.info.to.sup.table <- function(df.pls.line.info, df.sup.table, chr.sup.col.num.name) {
  ...
  df.table.merge <- cbind(df.sup.table,
                          merge.me = paste0(as.character(df.sup.table$sub),
                                            tolower(as.character(df.sup.table$PLS))))
  ...
  df.table.sup.out <- stats::reshape(df.table.sup[, ], timevar = "Month", direct = "wide",
